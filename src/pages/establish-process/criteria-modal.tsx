...
const Index = ({ open, handleCancel, update, id }: ModalPropType) => {
  ...
  const { mutateAsync: createItem, isPending: isCreating } =
    useApiMutation<any>({ url: "kpi-criterions", method: "POST" });
  const { mutateAsync: updateItem, isPending: isUpdating } =
    useApiMutation<any>({ url: "kpi-criterions", method: "PUT" });
    ...
    if (open) {
      if (update) {
        form.setFieldsValue({
          ...
          total_ratio: update?.total_ratio || 0
        });
      } else {
        form.resetFields();
      }
  ...
  return (
    <>
      <Modal
        open={open}
        title={update ? t("update_criteria") : t("add_criteria")}
      ...
      >
        ...
        >
          ...
          <Form.Item label={t("total_ratio")} name="total_ratio">
